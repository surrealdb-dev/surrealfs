...
|---|---|---:|---:|---|---|---|---|
| T1 | SurrealKV beta defects cause corruption or acknowledged-data loss | M | Critical | reopen mismatch, checksum errors, upstream critical fixes such as 0.21.3 compaction-fsync/memtable changes, flaky crash seeds | pinned revision; deterministic fault/compaction matrix; checksums/state roots; logical exports; support relationship | stop writes, quarantine, restore verified export/copy; stop or narrow product if systemic | Database lead |
| T2 | Nightly/private SurrealDB API or behavior changes rapidly | H | High | frequent compile/schema/query breakage; >10% capacity on upgrades | depend only on supported SDK; pin exact revision; adapter isolation; scheduled upgrades | hold known-good version; upstream/contract support; stop new releases if unsustainable | Database lead |
| T3 | SurrealDB transaction semantics do not cleanly support expected-head + root/graph commit | L/M | Critical | conflicts create partial records, non-repeatable concurrency tests | Phase 0 canonical-stack transaction proof; single semantic writer; receipt/invariant suite | redesign transaction encoding within the chosen stack; stop if atomicity cannot be proven | Storage lead |
...
| P2 | Database choice is mistaken for moat and is easily copied | H | High | roadmap dominated by engine; no unique integrations/recovery outcomes | treat SurrealDB/SurrealKV as fixed infrastructure; measure causal completeness, repeated recovery, trust, integration coverage | redirect investment to product loop; stop if only engine novelty remains | Product/architecture |
...
| ID | Risk | P | Impact | Leading indicators | Mitigation | Contingency | Owner |
|---|---|---:|---:|---|---|---|---|
| L1 | SurrealDB BSL terms conflict with distribution or managed offering | M | Critical | counsel flags service similarity/redistribution; license changes | Phase 0 legal opinion; exact version/feature/deployment review; commercial discussion | obtain commercial agreement, alter product/deployment, or stop | Legal/leadership |
| L2 | SurrealKV/SurrealDB support maturity is insufficient for SLA | M | High | beta persists; slow critical fixes; no support path | support agreement/upstream relationship; pinned fork capability; extensive validation | own patch branch temporarily; narrow the SLA/product or stop | Leadership/database |
...
| O3 | Benchmark results are misleading or irreproducible | M | High | laptop-only numbers, durability mismatch, missing raw output | standard harness/config manifests; reference-model equality; raw results and CI canaries | retract result, rerun independently | Reliability owner |
...
2. `T1/T3/T21/T22`: prove crash-safe root commit, conditional head movement, root/projection
   consistency, and public lifecycle on the pinned SurrealDB/SurrealKV stack.
